...
import GovPilotGridComponent from './GovPilotGrid';
import { comp1, comp4, comp3, comp5, comp6 } from './fixtures';

import ModalGovPilotGrid from '../../../test/forms/modalGovPilotGrid';
import Webform from '../../Webform';
import { displayAsModalGovPilotGrid } from '../../../test/formtest';

describe('GovPilotGrid Component', () => {
  it('Should set correct values in dataMap inside editGrid and allow aditing them', (done) => {
    Harness.testCreate(GovPilotGridComponent, comp4).then((component) => {
  ...
  it('Should display saved values if there are more then 1 nested components', (done) => {
    Harness.testCreate(GovPilotGridComponent, comp3).then((component) => {
  ...
  it('Should build an empty edit grid component', () => {
    return Harness.testCreate(GovPilotGridComponent, comp1).then((component) => {
  ...
  it('Should disable components when in read only', () => {
    return Harness.testCreate(GovPilotGridComponent, comp1, { readOnly: true }).then((component) => {
      Harness.testSetGet(component, [
        {
          field1: 'good',
          field2: 'foo'
  ...
  describe('Display As Modal', () => {
    it('Should show add error classes to invalid components', (done) => {
      ...
      form.setForm(displayAsModalGovPilotGrid).then(() => {
    ...
    it('Should set alert with validation errors on save', (done) => {
      const formElement = document.createElement('div');
      const form = new Webform(formElement);
      form.setForm(ModalGovPilotGrid).then(() => {
        const editGrid = form.components[0];
  ...
  describe('Draft Rows', () => {
    it('Check saving rows as draft', (done) => {
      Harness.testCreate(GovPilotGridComponent, comp5).then((component) => {
        component.addRow();
  ...
  //   return Harness.testCreate(GovPilotGridComponent, comp2).then(component => {
  //     Harness.clickElement(component, component.refs[`${component.editgridKey}-addRow`][0]);
