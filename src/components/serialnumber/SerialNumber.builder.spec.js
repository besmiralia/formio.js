...
describe('SerialNumber Builder', () => {
  ...
  it('Should create a new SerialNumber component', () => {
    builder = Harness.buildComponent('SerialNumber');
    return builder.editForm.formReady.then(() => {
      ...
      assert(preview.indexOf('formio-component formio-component-SerialNumber formio-component-SerialNumber') !== -1, 'Must have correct classes');
      assert(preview.indexOf('<label class="control-label" style="">Text Field</label>') !== -1, 'Must have a label');
      assert(preview.indexOf('<input name="data[SerialNumber]" type="text" class="form-control"') !== -1, 'Must have an input');
    });
  });
