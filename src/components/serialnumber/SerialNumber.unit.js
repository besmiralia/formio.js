...
import SerialNumberComponent from './SerialNumber';
...
describe('SerialNumber Component', () => {
  it('Should create a new SerialNumber', () => {
    const SerialNumber = new SerialNumberComponent({
      label: 'First Name',
      key: 'firstName',
      input: true,
      type: 'SerialNumber'
    ...
    assert.equal(SerialNumber.component.key, 'firstName');
  });

  it('Should build a SerialNumber component', () => {
    return Harness.testCreate(SerialNumberComponent, comp1).then((component) => {
      Harness.testElements(component, 'input[type="text"]', 1);
    });
  });

  it('Should disable multiple mask selector if component is disabled', (done) => {
    Harness.testCreate(SerialNumberComponent, comp4).then((component) => {
  ...
  it('Should provide required validation', () => {
    return Harness.testCreate(SerialNumberComponent, _.merge({}, comp2, {
  ...
  it('Should correctly calculate remaining words', (done) => {
    Harness.testCreate(SerialNumberComponent, comp5).then((component) => {
